...
Money Moment allowlist: age band, `kk|ru`, learning-objective enum, four basis-point percentages, and boolean/enum progress signals. It excludes names, identifiers, exact amounts, task/agreement/goal text, free text, contacts, and provider conversation state.
...
| Model data exfiltration/injection | enum-only input, server adapter, strict schema, safety filter, timeout/rate limit, deterministic fallback, `store:false`, no tools | schema/safety/fallback/non-exposure tests |
